...
class AnalysisThread(QThread):
    ...
    def __init__(self, files, html_detector, password_detector, image_analyzer, file_utils, blur_threshold):
        super().__init__()
        self.files = files
        ...
        for i, file_path in enumerate(self.files):
            try:
                ...
                if file_type == 'Image':
                    ...
                    result['Blur Score'] = f"{blur_score:.2f}"
...
class SmartFileAnalyzerUI(QMainWindow):
    def __init__(self, html_detector, password_detector, image_analyzer, file_utils):
        super().__init__()
        self.html_detector = html_detector
        self.password_detector = password_detector
        self.image_analyzer = image_analyzer
    ...
    def init_ui(self):
        ...
        self.results_table.setHorizontalHeaderLabels([
            "File Name", "Type", "HTML", "Protected", "Blurry", "Blur Score", "Cropped", "Status"
        ])
    ...
    def start_analysis(self):
        ...
        self.thread = AnalysisThread(
            ...
            self.image_analyzer, self.file_utils, self.blur_spin.value()
        )
    ...
    def update_progress(self, val, result):
        ...
        set_item(5, result.get('Blur Score', ''))
        set_item(6, result.get('Cropped', ''))
        set_item(7, result.get('Status', ''))
